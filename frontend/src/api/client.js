...
function isBusinessException(error) {
    if (!error) return false;
    const msg = error.message || '';
    // Postgres RAISE EXCEPTION from our RPCs returns Arabic business-rule
    // messages (e.g. "الموظف في إجازة بدون مرتب") with any HTTP status.
    // We must show them to the user as-is instead of hiding behind a generic
    // "الخادم غير متاح" — they are NOT server errors.
    return /[\u0600-\u06FF]/.test(msg);
}
