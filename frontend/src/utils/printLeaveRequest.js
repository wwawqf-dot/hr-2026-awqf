...
const FIELDS = {
    ...
    // leave), NOT the earned-to-date cut-off the rows above use.
...
export function buildLeaveRequestValues(employee, deduction, entitledBeforeDays) {
    ...
    // "إلى غاية" — the date the printed entitlement runs to. Days are
    // earned by serving the months, so this is the end of the last period
    // actually credited, not the year end. Printing 31/12 in March would
    // put the office's signature under a figure covering nine months the
    // employee has not served. Same helper the "مضاف حتى ..." labels use.
