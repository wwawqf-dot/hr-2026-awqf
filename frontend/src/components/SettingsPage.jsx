...
export default function SettingsPage({ leaveData }) {
    ...
    async function handleReconcile() {
        ...
        try {
            ...
            if (blocked.length) {
                setBackupError(
                    'تعذّر تصحيح رصيد الموظفين التالين لأن التصحيح كان سيجعل رصيدهم التراكمي سالباً ' +
                    '(أي أنهم استهلكوا أكثر مما يملكون، بعد احتساب الرصيد المرحّل)، ويحتاج الأمر لمراجعة يدوية: ' +
                    blocked
                        .map((b) => `${b.employee} (المسجّل ${b.stored} / الصحيح ${b.correct} / المخصوم ${b.deducted} / الرصيد بعد التصحيح ${b.net})`)
                        .join('، ')
                );
            }
        } catch (err) {
