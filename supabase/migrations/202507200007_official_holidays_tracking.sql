-- =====================================================================
--  Migration: Track official holidays subtracted during deductions
--  ---------------------------------------------------------------------
--  Adds an official_holidays column to the deductions table so the
--  system records how many public holidays were excluded from the
--  calendar-day range, making the net-days value auditable.
--
--  Changes:
--   1. ALTER TABLE  — add official_holidays (default 0 for existing)
--   2. get_employee_json — include officialHolidays in deductions_history
--   3. register_deduction — accept p_official_holidays, store it
--   4. create_employee  — include official_holidays in reconciliation
--   5. update_employee  — include official_holidays in reconciliation
--   6. bulk_add_employees — include official_holidays in reconciliation
--   7. sync_employees   — include official_holidays in both insert paths
-- =====================================================================

alter table public.deductions
    add column if not exists official_holidays numeric not null default 0;

-- ---------------------------------------------------------------------
--  get_employee_json – include officialHolidays
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
--  register_deduction – accept & store official_holidays
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
--  create_employee – include official_holidays in reconciliation insert
-- ---------------------------------------------------------------------
...
begin
    ...
    if v_has_remaining and v_current is not null then
        ...
        if v_consumed > 0 then
            ...
            insert into public.deductions (employee_id, year, start_date, end_date, days, note, created_by, created_at, official_holidays)
            values (v_new_id, v_current, '', '', v_consumed, v_recon_note, v_username, now(), 0);
        end if;
    end if;
...
-- ---------------------------------------------------------------------
--  update_employee – include official_holidays in reconciliation insert
-- ---------------------------------------------------------------------
    ...
    if v_has_remaining then
        ...
        if v_current is not null then
            ...
            if v_diff > 0 then
                ...
                insert into public.deductions (employee_id, year, start_date, end_date, days, note, created_by, created_at, official_holidays)
                values (p_id, v_current, '', '', v_diff, v_recon_note, v_username, now(), 0);
                v_recon_days := v_diff;
            end if;
        end if;
    end if;
...
-- ---------------------------------------------------------------------
--  bulk_add_employees – include official_holidays in reconciliation
-- ---------------------------------------------------------------------
...
begin
    ...
    for r in select value from jsonb_array_elements(p_rows) loop
        ...
        if nullif(trim(coalesce(r->>'remainingBalance','')),'') is not null and v_current is not null then
            ...
            v_consumed  := 30 - v_remaining;
            if v_consumed > 0 then
                update public.employee_years set deducted = deducted + v_consumed
                    where employee_id = v_id and year = v_current;
                insert into public.deductions (employee_id, year, start_date, end_date, days, note, created_by, created_at, official_holidays)
                values (v_id, v_current, '', '', v_consumed, v_note, v_username, now(), 0);
...
-- ---------------------------------------------------------------------
--  sync_employees – include official_holidays in both insert paths
-- ---------------------------------------------------------------------
            ...
            for ded in select value from jsonb_array_elements(emp->'deductions_history') loop
                ...
                if nullif(ded->>'id','') is not null then
                    insert into public.deductions
                        (id, employee_id, year, start_date, end_date, days, note, created_by, created_at, official_holidays)
                    values ((ded->>'id')::bigint, v_target, coalesce(ded->>'year',''),
                            ...
                            coalesce((ded->>'officialHolidays')::numeric, 0))
                    on conflict (id) do nothing;
                else
                    insert into public.deductions
                        (employee_id, year, start_date, end_date, days, note, created_by, created_at, official_holidays)
