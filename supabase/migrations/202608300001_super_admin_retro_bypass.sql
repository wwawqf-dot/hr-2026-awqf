-- =====================================================================
--  Super Admin = wqf-837dec4f6914@internal.local (عبد الرحيم)
--  ---------------------------------------------------------------------
--  1. Point the super-admin identity at the internal account email.
--     The same constant drives every super-admin protection path
--     (never demote / never delete / never edit), so this single change
--     relocates root status to the new account.
--
--  2. Allow ONLY the Super Admin to record a dated deduction at any
--     point back to the start of the (financial) year in which the leave
--     starts — i.e. they may register leave retroactively all the way to
--     the start of the year, even near the end of the year. Everyone
--     else keeps the existing 40-day retroactive limit.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. SUPER ADMIN IDENTITY
-- ---------------------------------------------------------------------
-- Two copies of this constant exist historically. The live protection
-- functions (protect_super_admin, update_user_role, delete_auth_user) all
-- read public.super_admin_email(); app_constants.super_admin_email() is the
-- older copy. Update BOTH so the whole system agrees on the root account,
-- and so register_deduction below can use the same canonical helper.
...
-- ---------------------------------------------------------------------
-- 2. register_deduction WITH SUPER-ADMIN RETRO BYPASS
-- ---------------------------------------------------------------------
...
begin
    ...
    v_is_super_admin := (v_email = public.super_admin_email());
    ...
    if v_has_dates then
        ...
        -- Retroactive bound. The Super Admin may record a dated leave at
        -- any point back to the start of the financial year it belongs to
        -- (even a full year back when we are near the end of the year);
        -- everyone else is limited to the 40-day window.
