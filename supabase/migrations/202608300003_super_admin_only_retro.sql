-- =====================================================================
--  Correct: only the SUPER ADMIN gets full-year retroactive leave
--  ---------------------------------------------------------------------
--  202608300002 inadvertently gave the whole-year privilege to every
--  'admin'-role account. This migration restores it to the SUPER ADMIN
--  ONLY (identified by public.super_admin_email() — عبد الرحيم). Regular
--  administrators and data_entry both stay bound to the 40-day window.
-- =====================================================================
...
begin
    ...
    -- ONLY the Super Admin (عبد الرحيم) may record a dated leave at any
    -- point in the financial year. Every other account — including other
    -- 'admin' users and everything of lower rank — stays bound to the
    -- 40-day retroactive window.
    ...
    if v_has_dates then
        ...
        -- Retroactive bound. ONLY the Super Admin (عبد الرحيم) may record
        -- a dated leave at any point back to the start of the financial
        -- year it belongs to. Everyone else — including other 'admin'
        -- accounts — is limited to the 40-day window.
