-- =====================================================================
--  Migration: Add deduction_source column + FIFO tracking in RPC
-- =====================================================================

-- 1. Add deduction_source column to deductions table
alter table public.deductions
    add column if not exists deduction_source text default null;

-- 2. Update register_deduction to calculate and store FIFO source
...
begin
    ...
    -- FIFO source: calculate carry-over from years BEFORE v_year
    select coalesce(emp.initial_carried_forward, 0)
         + coalesce(sum(coalesce(added,0) - coalesce(deducted,0)), 0)
      into v_prev_carry
    ...
    if v_prev_carry <= 0 then
        v_fifo_source := 'حالي';
    elsif v_days <= v_prev_carry then
        v_fifo_source := 'سابق';
    else
        v_fifo_source := 'موزع';
    end if;
...
-- 3. Update get_employee_json to include deduction_source in history
