-- =====================================================================
--  Migration: Unify weekend exclusion (Friday + Saturday for everyone)
--  1. Redefine calculate_deduction_days with the boolean parameter
--     dropped — weekend is now always [5,6] (Fri+Sat). Thursday is a
--     regular work day for all employees.
--  2. Strip is_memorizer out of every function that reads/writes it
--     (get_employee_json, create_employee, update_employee,
--     sync_employees, register_deduction's call site).
--  3. Drop the now-unused is_memorizer column.
--  Functions are redefined BEFORE the column drop so nothing references
--  a dropped column at the point it's removed.
-- =====================================================================

-- Old 4-arg signature (p_is_memorizer) is being retired entirely — drop
-- it explicitly so CREATE FUNCTION below defines a clean 3-arg overload
...
-- ---------------------------------------------------------------------
-- Column is only dropped now that no function body references it.
alter table public.employees drop column if exists is_memorizer;
